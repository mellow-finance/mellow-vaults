...
contract BasePulseStrategyUpgradable is ContractMeta, DefaultAccessControlLateInit, ILpCallback {
    ...
    function setRouter(address newRouter) external {
        ...
        ImmutableParams memory params = immutableParams;
        for (uint256 i = 0; i < params.tokens.length; i++) {
            address token = params.tokens[i];
            params.erc20Vault.externalCall(token, IERC20.approve.selector, abi.encode(params.router, 0));
            params.erc20Vault.externalCall(token, IERC20.approve.selector, abi.encode(newRouter, type(uint256).max));
        }
        immutableParams.router = newRouter;
    }
    ...
    function rebalance(
        ...
        Interval memory interval = _positionsRebalance(immutableParams_, newInterval, pool);
    ...
    function checkMutableParams(MutableParams memory params) public pure {
    ...
    function _positionsRebalance(
        ...
        bool isSameInterval = compareIntervals(newInterval, uniV3Nft);
    ...
    function _swapToTarget(
        ...
        require(
            actualAmountOut >= minAmountOutInCaseOfSwap,
            string(
                abi.encodePacked(
                    "Limit overflow: expected offchain amountOut (",
                    Strings.toString(minAmountOutInCaseOfSwap),
                    "), actual amountOut (",
                    Strings.toString(actualAmountOut),
                    ")"
                )
            )
        );
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "Limit overflow: expected swap priceX96 (",
                    Strings.toString(priceX96),
                    "), actual swap priceX96 (",
                    Strings.toString(actualSwapPriceX96),
                    ")"
                )
            )
        );
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "Limit overflow: expected amountIn (",
                    Strings.toString(amountIn),
                    "), actual amountIn (",
                    Strings.toString(actualAmountIn),
                    ")"
                )
            )
        );
    ...
    function _contractName() internal pure override returns (bytes32) {
        return bytes32("BasePulseStrategyUpgradable");
    }
