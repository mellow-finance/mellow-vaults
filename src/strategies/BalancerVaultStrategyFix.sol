...
contract BalancerVaultStrategyFix is ContractMeta, ILpCallback, DefaultAccessControlLateInit {
    ...
    function liquidityWithdrawal(bytes memory balancerVaultData) external {
        _requireAdmin();
        try
            IIntegrationVault(subvault).externalCall(
                0x2279abf4bdAb8CF29EAe4036262c62dBA6460306,
                0x2e1a7d4d,
                abi.encode(IERC20(0x2279abf4bdAb8CF29EAe4036262c62dBA6460306).balanceOf(address(subvault)))
            )
    ...
    function _contractName() internal pure override returns (bytes32) {
        return bytes32("BalancerVaultStrategyFix");
    }
