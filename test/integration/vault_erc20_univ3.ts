  ...
  function () {
    ...
    describe("rebalance", () => {
      it("initializes uniV3 vault with position nft and increases tvl respectivly", async () => {
        ...
        expect(await this.uniV3Vault.uniV3Nft()).to.deep.equal(result.tokenId);
      ...
      it("deposits", async () => {
        ...
        await this.subject.deposit(
          [BigNumber.from(10).pow(6).mul(3000), BigNumber.from(10).pow(18)],
          0
        );
        expect(
      ...
      it("pulls univ3 to erc20 and collects earnings", async () => {
        ...
        console.log("uniV3Vault tvl", (await this.uniV3Vault.tvl()).toString());
      });
