...
describe("LpIssuerGovernance", () => {
    ...
    describe("constructor", () => {
        it("deploys", async () => {
            expect(contract.address).to.not.be.equal(ethers.constants.AddressZero);
        });
    });
    ...
    describe("strategyTreasury", () => {
        it("treasury == 0x0", async () => {
            ...
            expect(await contract.strategyTreasury(nft)).to.be.equal(ethers.constants.AddressZero);
        });
    });

    describe("setDelayedStrategyParams", () => {
        it("sets strategy params and emits SetStrategyParams event", async () => {
            let nft = Math.random() * (2 ** 52);
            let tokenLimit = Math.random() * (2 ** 52);
            await expect(await contract.setDelayedStrategyParams(nft, {tokenLimitPerAddress: tokenLimit})).to.emit(contract, "SetStrategyParams");
        });
    });
    ...
    describe("strategyParams", () => {
        it("returns correct strategy params", async () => {
            let nft = Math.random() * (2 ** 52);
            let tokenLimit = Math.random() * (2 ** 52);
            await contract.setStrategyParams(nft, {tokenLimitPerAddress: tokenLimit});
