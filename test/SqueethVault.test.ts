    ...
    function () {
        before(async () => {
            this.deploymentFixture = deployments.createFixture(
                async (_, __?: DeployOptions) => {
                    ...
                    for (let address of [this.subject.address]) {
                        await mint(
                            "WETH",
                            address,
                ...
                await withSigner(
                    ...
                    async (signer) => {
                        await expect(
                            this.subject
                                .connect(signer)
                                .initialize(this.nft, [this.weth.address])
            ...
            describe("edge cases:", () => {
                ...
                describe("when there are two tokens", () => {
                    it(`reverts with ${Exceptions.INVALID_LENGTH}`, async () => {
                        await expect(
                            this.subject.initialize(this.nft, [
                                this.weth.address,
                                this.squeethWPowerPerp.address,
                            ])
                ...
                describe("when setting zero nft", () => {
                    it(`reverts with ${Exceptions.VALUE_ZERO}`, async () => {
                        await expect(
                            this.subject.initialize(0, [this.weth.address])
        ...
        describe("#takeShort", () => {
            it("mints wPowerPerp using entire weth supply as a collateral", async () => {
                let wethBalance = await this.weth.balanceOf(
                    this.subject.address
                );
                ...
                expect(
                    (
                        await this.squeethWPowerPerp.balanceOf(
                            this.subject.address
                        )
                    ).eq(0)
                ).to.be.true;
                ...
                let mintedSqueeth = await this.squeethWPowerPerp.balanceOf(
                    this.subject.address
                );
                expect((await this.subject.shortVaultId()).gt(0)).to.be.true;
                expect((await this.subject.totalCollateral()).eq(wethBalance))
                    .to.be.true;
                expect((await this.subject.wPowerPerpDebt()).eq(mintedSqueeth))
                    .to.be.true;
                expect(mintedSqueeth.gt(0)).to.be.true;
                expect((await this.weth.balanceOf(this.subject.address)).eq(0))
                    .to.be.true;
            });
            ...
            it("opens short position using all resources as collateral", async () => {
                ...
                expect((await this.subject.shortVaultId()).gt(0)).to.be.true;
                expect((await this.subject.totalCollateral()).gt(wethBalance))
                    .to.be.true;
                ...
                expect(
                    (await this.weth.balanceOf(this.subject.address)).lt(
                        wethBalance
                    )
                ).to.be.true;
            });
            ...
            it("cant open short position with recollaterization when pools are mad", async () => {
                ...
                await uniSwapTokensGivenInput(
                    this.swapRouter,
                    [this.squeethWPowerPerp, this.weth],
                    3000,
                    true,
                ...
                await expect(
                    this.subject.takeShort(this.healthFactor, true)
            ...
            it("opens usual short position when pools are mad", async () => {
                ...
                await expect(this.subject.takeShort(this.healthFactor, false))
                    .to.not.be.reverted;
            });

            it("emits ShortTaken event", async () => {
                await expect(
                    this.subject.takeShort(this.healthFactor, false)
                ...
                it("allowed: vault owner", async () => {
                    await expect(
                        this.subject
                            ...
                            .takeShort(this.healthFactor, false)
                    ).to.not.be.reverted;
                });
        ...
        describe("#closeShort", () => {
            it("closes position using oSQTH left on vault", async () => {
                ...
                let squeethBalance = await this.squeethWPowerPerp.balanceOf(
                    this.subject.address
                );
                expect((await this.subject.wPowerPerpDebt()).eq(squeethBalance))
                    .to.be.true;
                ...
                expect((await this.weth.balanceOf(this.subject.address)).gt(0))
                    .to.be.true;
            });
            ...
            it("emits ShortClosed event", async () => {
                await this.subject.takeShort(this.healthFactor, false);
                await expect(await this.subject.closeShort()).to.emit(
                    this.subject,
                    "ShortClosed"
                );
            });
            ...
            describe("access control:", () => {
                it("allowed: vault owner", async () => {
                    await this.subject.takeShort(this.healthFactor, false);
                    await expect(
                        this.subject.connect(this.deployer).closeShort()
                    ).to.not.be.reverted;
                });
                ...
                it("allowed: approved account", async () => {
                    ...
                    await withSigner(account, async (s) => {
                        await expect(this.subject.connect(s).closeShort()).to
                            .not.be.reverted;
                    });
                });
                ...
                it("denied: any other address", async () => {
                    ...
                    await withSigner(randomAddress(), async (s) => {
                        await expect(
                            this.subject.connect(s).closeShort()
                        ).to.be.revertedWith(Exceptions.FORBIDDEN);
                    });
                });
            });
