...
contract PancakePulseV2Test is Test {
    ...
    function deployGovernances() public {
        pancakeSwapVaultGovernance = new PancakeSwapVaultGovernance(
            IVaultGovernance.InternalParams({
                protocolGovernance: IProtocolGovernance(governance),
                registry: IVaultRegistry(registry),
                singleton: IVault(address(new PancakeSwapVault()))
            }),
            IPancakeSwapVaultGovernance.DelayedProtocolParams({
        ...
        vm.startPrank(admin);
        IProtocolGovernance(governance).stagePermissionGrants(address(pancakeSwapVaultGovernance), new uint8[](1));
    ...
    function actualRewardsAmount() public returns (uint256) {
        return pancakeSwapVault.compound();
    }
