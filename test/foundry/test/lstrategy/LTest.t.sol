...
contract LStrategyTest is Test {
    ...
    function deposit(uint256 amount) public {
        ...
        deal(wsteth, deployer, amount * 10**15);
    ...
    function setupSecondPhase() public payable {
        ...
        deal(weth, deployer, 5*10**17);
        deal(wsteth, deployer, 5*10**17);

        IERC20(weth).transfer(address(lstrategy), 3 * 10**17);
        IERC20(wsteth).transfer(address(lstrategy), 3 * 10**17);
...
*/
    ...
    function changePrice(int24 needTick) public {
        IUniswapV3Pool pool = uniV3LowerVault.pool();

        uint256 startA = 10**18;
        uint256 startB = 10**18;
        ...
        while (true) {
            t += 1;
            (, int24 tick, , , , , ) = uniV3LowerVault.pool().slot0();
            ...
            if (tick > needTick) {
                ...
                bytes memory b = "";
                deal(wsteth, deployer, startA);

                IERC20(wsteth).approve(uniswapV3Router, startA);
                ...
                bytes memory b = "";
                deal(weth, deployer, startB);

                IERC20(weth).approve(uniswapV3Router, startB);
    ...
    function testDeposit() external {
        ...
        (uint256[] memory lTvl, ) = uniV3LowerVault.tvl();
        (uint256[] memory uTvl, ) = uniV3UpperVault.tvl();
        deposit(100);
        (uint256[] memory erc20TvlA, ) = erc20Vault.tvl();
        (uint256[] memory lTvlA, ) = uniV3LowerVault.tvl();
        (uint256[] memory uTvlA, ) = uniV3UpperVault.tvl();

        uint256 Z = FullMath.mulDiv(lTvlA[0], 10**18, lTvl[0]);
        require(isClose(lTvl[0] * Z, lTvlA[0] * 10**18, 1000));
        require(isClose(lTvl[1] * Z, lTvlA[1] * 10**18, 1000));
        require(isClose(uTvl[0] * Z, uTvlA[0] * 10**18, 1000));
        require(isClose(uTvl[1] * Z, uTvlA[1] * 10**18, 1000));
        require(isClose(erc20Tvl[0] * Z, erc20TvlA[0] * 10**18, 2));
        require(isClose(erc20Tvl[1] * Z, erc20TvlA[1] * 10**18, 2));
    }
    ...
    function testWithdraw() external {
        ...
        uint256 oldA = IERC20(weth).balanceOf(deployer);
        uint256 oldB = IERC20(wsteth).balanceOf(deployer);

        {
            ...
            {
                uint256 x = 0;
                bytes memory withdrawInfo = abi.encode(x, x, x, x);
                bytesArray[0] = withdrawInfo;
            }

            rootVault.withdraw(deployer, lpTokens/2, minTokenAmounts, bytesArray);
        ...
        uint256 newA = IERC20(weth).balanceOf(deployer);
        uint256 newB = IERC20(wsteth).balanceOf(deployer);

    
        require(isClose(newB - oldB, erc20Tvl[0] + lTvl[0] + uTvl[0] - ZA, 1000));
        require(isClose(newA - oldA, erc20Tvl[1] + lTvl[1] + uTvl[1] - ZB, 1000));
    }

    function testDepositChangeTick() public {
    ...
    function testWithdrawChangeTick() public {
