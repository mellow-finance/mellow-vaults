...
import "../interfaces/vaults/IAMMVault.sol";
...
contract ERC20RetroRootVaultGovernance is ContractMeta, IERC20RetroRootVaultGovernance, VaultGovernance {
    /// @inheritdoc IERC20RetroRootVaultGovernance
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return super.supportsInterface(interfaceId) || type(IERC20RetroRootVaultGovernance).interfaceId == interfaceId;
    }
    ...
    function createVault(
        ...
        address owner_
    ) external returns (IERC20RetroRootVault vault, uint256 nft) {
        address vaddr;
        IVaultRegistry registry = _internalParams.registry;
        (vaddr, nft) = _createVault(owner_);
        vault = IERC20RetroRootVault(vaddr);
        if (vaultTokens_.length != 2 || subvaultNfts_.length != 2) {
        ...
        address ammSubvault = _internalParams.registry.vaultForNft(subvaultNfts_[1]);
        address pool = IAMMVault(ammSubvault).pool();
        require(pool != address(0), ExceptionsLibrary.ADDRESS_ZERO);
        vault.initialize(nft, vaultTokens_, strategy_, subvaultNfts_, helper, pool);
    }

    // -------------------  INTERNAL, VIEW  -------------------

    function _contractName() internal pure override returns (bytes32) {
        return bytes32("ERC20RetroRootVaultGovernance");
    }
