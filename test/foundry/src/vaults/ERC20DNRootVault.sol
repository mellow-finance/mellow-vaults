...
contract ERC20DNRootVault is IERC20RootVault, ERC20Token, ReentrancyGuard, AggregateVault {
    ...
    function tvl() public view override(IVault, AggregateVault) returns (uint256[] memory minTokenAmounts, uint256[] memory maxTokenAmounts) {
        ...
        for (uint256 i = 0; i < 3; ++i) {
            (uint256[] memory minSubvaultTvl, uint256[] memory maxSubvaultTvl) = IIntegrationVault(IAggregateVault(address(this)).subvaultAt(i)).tvl();
