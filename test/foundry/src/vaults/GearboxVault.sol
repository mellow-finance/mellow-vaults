...
contract GearboxVault is IGearboxVault, IntegrationVault {
    ...
    function _pull(
        ...
        _helper.withdrawFromConvex(IERC20(convexOutputToken).balanceOf(creditAccount), address(_vaultGovernance), poolId, primaryIndex);
        ...
        if (underlyingBalance < debtAmount + 1) {
            _helper.swapExactOutput(depositToken, primaryToken, debtAmount + 1 - underlyingBalance, 0, address(_vaultGovernance), creditAccount);
        }
        ...
        if (depositTokenBalance < amountToPull && primaryToken != depositToken) {
            _helper.swapExactOutput(primaryToken, depositToken, amountToPull - depositTokenBalance, debtAmount + 1, address(_vaultGovernance), creditAccount);
        }
    ...
    function _adjustPosition(uint256 expectedAllAssetsValue, uint256 currentAllAssetsValue) internal {
        ...
        _helper.checkNecessaryDepositExchange(FullMath.mulDiv(expectedAllAssetsValue, D9, marginalFactorD9), address(_vaultGovernance), creditAccount);
