...
contract GearboxRootVault is IGearboxRootVault, ERC20Token, ReentrancyGuard, AggregateVault {
    ...
    function deposit(
        ...
        if (totalSupply == 0) {
            ...
            require(
                tokenAmounts[0] >= 10 * pullExistentialsForToken &&
    ...
    function registerWithdrawal(uint256 lpTokenAmount) external returns (uint256 amountRegistered) {
        ...
        if (!(currentEpoch == userLatestRequestEpoch || userLatestRequestEpoch == 0)) {
            _processHangingWithdrawal(msg.sender);
        }
    ...
    /// @inheritdoc IGearboxRootVault
    function cancelWithdrawal(uint256 lpTokenAmount) external returns (uint256) {
        ...
        if (currentWithdrawalRequests < lpTokenAmount) {
            lpTokenAmount = currentWithdrawalRequests;
        }
    ...
    function invokeExecution() public {
        ...
        require(lastEpochChangeTimestamp + params.withdrawDelay <= block.timestamp || isClosed, ExceptionsLibrary.INVARIANT);
        if (lastEpochChangeTimestamp + 2 * params.withdrawDelay > block.timestamp && !isClosed) {
            _requireAtLeastStrategy();
            _checkIsPoolOkay();
        }
        ...
        emit ExecutionInvoked(msg.sender, withdrawn);
    }
    ...
    function _processHangingWithdrawal(address addr) internal {
        ...
        uint256 tokenAmount = FullMath.mulDiv(pendingRequest, epochToPriceForLpTokenD18[latestRequestEpoch[addr]], D18);
    ...
    function _makeWithdraw(
        ...
        if (currentEpoch != userLatestRequestEpoch && userLatestRequestEpoch != 0) {
            _processHangingWithdrawal(user);
        }
    ...
    function _checkIsPoolOkay() internal {
        ...
        (, int256 priceAI, , ,) = IAggregatorV3(params.priceFeed).latestRoundData();
        ...
        if (priceA < priceB) {
            require(priceA * D18 > priceB * params.minPoolDeltaD18, ExceptionsLibrary.INVARIANT);
        }
        else {
            require(priceB * D18 > priceA * params.minPoolDeltaD18, ExceptionsLibrary.INVARIANT);
        }
