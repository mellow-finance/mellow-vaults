...
contract KyberPulseStrategyV2Helper {
    ...
    {
        {
            (address router, IERC20Vault erc20Vault, IKyberVault kyberVault, IOracle mellowOracle) = strategy
                .immutableParams();
            immutableParams = KyberPulseStrategyV2.ImmutableParams({
        ...
        }
        {
            ...
            mutableParams = KyberPulseStrategyV2.MutableParams({
    ...
    function calculateAmountForSwap(
        ...
        IPool pool = immutableParams.kyberVault.pool();
        ...
        {
            (KyberPulseStrategyV2.Interval memory interval, bool neededNewInterval) = strategy.calculateNewPosition(
                mutableParams,
                spotTick,
                pool,
                immutableParams.kyberVault.kyberNft()
            );

            if (neededNewInterval) {
                (delta, ) = immutableParams.kyberVault.tvl();
            }
