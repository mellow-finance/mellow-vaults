...
import { encodeToBytes, generateSingleParams, randomAddress, sleep, withSigner } from "./library/Helpers";
...
contract<UniV3Validator, DeployOptions, CustomContext>("UniV3Validator", function () {
    ...
    before(async () => {
        this.deploymentFixture = deployments.createFixture(
            async (_, __?: DeployOptions) => {
                ...
                this.EXACT_INPUT_SINGLE_SELECTOR = await this.subject.EXACT_INPUT_SINGLE_SELECTOR();
                this.EXACT_INPUT_SELECTOR = await this.subject.EXACT_INPUT_SELECTOR();
                this.EXACT_OUTPUT_SINGLE_SELECTOR = await this.subject.EXACT_OUTPUT_SINGLE_SELECTOR();
                this.EXACT_OUTPUT_SELECTOR = await this.subject.EXACT_OUTPUT_SELECTOR();
                
                const vaultTokens = [this.dai.address, this.usdc.address];
                let vaultOwner = randomAddress();
                const { vault } = await this.erc20VaultGovernance.connect(this.admin).callStatic.createVault(vaultTokens, vaultOwner);
                await this.erc20VaultGovernance.connect(this.admin).createVault(vaultTokens, vaultOwner);
    ...
    describe("#validate", () => {
        ...
        describe("EXACT_INPUT_SINGLE_SELECTOR", async () => {
            ...
            it("reverts if not a vault token", async () => {
                await withSigner(this.erc20RootVaultSingleton.address, async (signer) => {
                    await expect(
                        this.subject
                            .connect(signer)
                            .validate(
            ...
            it("pass", async () => {
                let pool = await this.uniswapV3Factory.connect(this.admin).callStatic.getPool(this.dai.address, this.usdc.address, this.fee);
                this.protocolGovernance.connect(this.admin).stagePermissionGrants(pool, [PermissionIdsLibrary.ERC20_APPROVE])
        ...
        describe("EXACT_INPUT_SELECTOR", async () => {
            ...
            it("reverts if tokens are the same", async () => {
                ...
                let path = Buffer.concat([token, randomBytes(3), token]); 
                await withSigner(randomAddress(), async (signer) => {
                    await expect(
                        this.subject
                            .connect(signer)
            ...
            it("reverts if not a vault token", async () => {
                let pool = await this.uniswapV3Factory.connect(this.admin).callStatic.getPool(this.dai.address, this.weth.address, this.fee);
                this.protocolGovernance.connect(this.admin).stagePermissionGrants(pool, [PermissionIdsLibrary.ERC20_APPROVE])
                await sleep(await this.protocolGovernance.governanceDelay())
                this.protocolGovernance.connect(this.admin).commitAllPermissionGrantsSurpassedDelay()
