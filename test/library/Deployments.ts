...
export const deployERC20Tokens = deployments.createFixture(
  async (
    ...
    // defaults<
    let constructorArgs: ERC20Test_constructorArgs[] =
...
export const deployProtocolGovernance = async (options?: {
  ...
  // defaults<
  const params: ProtocolGovernance_Params = options?.constructorArgs
    ?.params ?? {
  ...
  const contract = await Contract.connect(
    options?.adminSigner ?? (await ethers.getSigners())[0]
  ).deploy(constructorArgs.admin, constructorArgs.params);
  return contract;
};
...
export const deployVaultManagerGovernance = deployments.createFixture(
  async (
    _: HardhatRuntimeEnvironment,
    options?: {
...
export const deployERC20VaultManager = deployments.createFixture(
  async (
    _: HardhatRuntimeEnvironment,
    options?: {
...
export const deployERC20VaultFactory = deployments.createFixture(
  async (
    _: HardhatRuntimeEnvironment,
    __?: {
...
export const deployERC20VaultFromVaultManager = deployments.createFixture(
  async (
    _: HardhatRuntimeEnvironment,
    options?: {
...
export const deployCommonLibraryTest = deployments.createFixture(
  async (_: HardhatRuntimeEnvironment) => {
 ...
 * @dev From scratch.
 */
export const deployERC20VaultSystem = deployments.createFixture(
  async (
    _: HardhatRuntimeEnvironment,
    options?: {
    ...
    const erc20VaultFactory: ERC20VaultFactory =
      await deployERC20VaultFactory();
    ...
    ({ vaultGovernance, erc20Vault, nft } =
      await deployERC20VaultFromVaultManager({
        constructorArgs: {
