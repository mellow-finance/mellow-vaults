...
(async() => {
	...
	let swapAmount = await oweth.balanceOf(arbitrager.address);
	let maxPriorityFeePerGas = BigNumber.from(
		await provider.send("eth_maxPriorityFeePerGas", [])
	  );
	
	await oweth.connect(arbitrager).approve(swapRouter.address, swapAmount);

	let tx = await swapRouter.connect(arbitrager).exactInputSingle({
		tokenIn: process.env.OWETH_ADDRESS,
		tokenOut: process.env.OUSDC_ADDRESS,
		fee: 3000,
