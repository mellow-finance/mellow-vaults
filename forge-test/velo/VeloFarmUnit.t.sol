...
contract Integration is Test {
    ...
    function testInitialize() external {
        ...
        farm.initialize(address(0), address(0), address(0), address(0), 0);

        vm.expectRevert(abi.encodePacked("AZ"));
        farm.initialize(address(vaultInfo.rootVault), address(0), address(0), address(0), 0);

        vm.expectRevert(abi.encodePacked("AZ"));
        farm.initialize(address(vaultInfo.rootVault), address(deployer), address(0), address(0), 0);

        vm.expectRevert(abi.encodePacked("AZ"));
        farm.initialize(address(vaultInfo.rootVault), address(deployer), address(protocolTreasury), address(0), 0);

        address rewardToken = gauge.rewardToken();
        vm.expectRevert(abi.encodePacked("FRB"));
        farm.initialize(
            address(vaultInfo.rootVault),
            address(deployer),
    ...
    function testGetStorage() external {
        VeloFarm farm = VeloFarm(vaultInfo.farm);
        VeloFarm.Storage memory s = farm.getStorage();
        assertEq(s.lpToken, address(vaultInfo.rootVault));
        assertEq(s.rewardToken, address(gauge.rewardToken()));
        assertEq(s.protocolTreasury, address(protocolTreasury));
        assertEq(s.protocolFeeD9, protocolFeeD9);
        assertEq(s.totalCollectedAmounts, 0);
        assertEq(s.totalClaimedAmounts, 0);
        assertEq(
            s.name,
            string(abi.encodePacked(IERC20Metadata(address(vaultInfo.rootVault)).name(), " instant farm"))
        );
        assertEq(s.symbol, string(abi.encodePacked(IERC20Metadata(address(vaultInfo.rootVault)).symbol(), "IF")));
    }
