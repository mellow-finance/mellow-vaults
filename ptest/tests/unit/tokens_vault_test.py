...
def test_constructor(a, tokens, order, TokensVault):
...
def test_constructor_repeated_tokens(a, tokens, order, TokensVault):
    addresses = [str(tokens[i]).lower() for i in order]
    with reverts("TE"):
        TokensVault.deploy(addresses, {"from": a[0]})


def test_constructor_empty_tokens(a, TokensVault):
    with reverts("ETL"):
        TokensVault.deploy([], {"from": a[0]})


@pytest.mark.parametrize("order", [[2, 1, 0], [0, 1]])
def test_tokens(a, tokens, order, TokensVault):
    addresses = [str(tokens[i]).lower() for i in order]
    tokens_vault = TokensVault.deploy(addresses, {"from": a[0]})
    assert tokens_vault.tokens() == sorted(addresses)
...
def test_tokens_count(a, tokens, order, TokensVault):
    addresses = [str(tokens[i]).lower() for i in order]
    tokens_vault = TokensVault.deploy(addresses, {"from": a[0]})
    assert tokens_vault.tokensCount() == len(addresses)


def test_own_token_amounts(a, tokens, TokensVault):
...
def test_has_token(a, tokens, TokensVault):
    tokens_vault = TokensVault.deploy(tokens, {"from": a[0]})
    for token in tokens:
