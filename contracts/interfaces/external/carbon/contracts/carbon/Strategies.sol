...
abstract contract Strategies is Initializable {
    ...
    mapping(uint128 => EnumerableSet.UintSet) private _strategyIdsByPairIdStorage;
     ...
     */
    function __Strategies_init() internal {
     ...
     */
    function __Strategies_init_unchained() internal {
    ...
    function _updateStrategy(
        ...
        for (uint256 i = 0; i < 2; i = uncheckedInc(i)) {
            ...
            if (value > 0 && newOrders[i].y <= orders[i].y) {
    ...
    function _strategiesByPair(
        ...
        EnumerableSet.UintSet storage strategyIds = _strategyIdsByPairIdStorage[pair.id];
     ...
     */
    function _withdrawFunds(Token, address payable target, uint256 amount) private {
        if (amount == 0) {
            return;
        }
