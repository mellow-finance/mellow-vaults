...
interface IPerpLPVault is IIntegrationVault {
    /// @notice Options for the operations with the Uni position
    /// @param deadline The restriction on when the transaction should be executed, otherwise, it fails
    struct Options {
        uint256 deadline;
    }

    /// @notice Main information, representing the UniV3 position
    ...
    /// @notice Returns the UniswapV3 pool, which is the underlying pool with vUSDC / baseToken token pair and 0,3% fees
    ...
    /// @notice Flag, representing the status of the Uni position (open / close)
    /// @return bool Returns true if position is opened, else - false
    ...
    /// @notice Opens a position on the UniswapV3 pool using the given tick boundaries, and to provide liquidity to the pool
