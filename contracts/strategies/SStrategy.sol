...
contract SStrategy is ContractMeta, DefaultAccessControl {
    ...
    function startCycleMocked(
        uint256 strike,
        uint256 optionPriceUSD,
        address safe,
        bool reusePerp
    ) external {
        _requireAtLeastOperator();
        require(startingTime == 0, ExceptionsLibrary.INVALID_STATE);
        require(rootVault.isClosed() == false, ExceptionsLibrary.INVALID_STATE);

        uint256 currentPrice = squeethVault.twapIndexPrice();
        uint256 strikeD9 = FullMath.mulDiv(strike, D9, currentPrice);
        uint256 amountD9 = FullMath.mulDiv(
            strategyParams.upperHedgingThresholdD9 - D9,
            strategyParams.upperHedgingThresholdD9 - strikeD9,
            D9
        );
        {
    ...
    function endCycleMocked(address safe) external returns (uint256 priceChangeD9, bool isLiquidated) {
        _requireAtLeastOperator();
        require(startingTime != 0, ExceptionsLibrary.INVARIANT);

        uint256 currentPrice = squeethVault.twapIndexPrice();
        priceChangeD9 = FullMath.mulDiv(currentPrice, D9, startingPrice);

        if (rootVault.isClosed() == false) {
            if (
                priceChangeD9 < liquidationParams.lowerLiquidationThresholdD9 ||
                priceChangeD9 > liquidationParams.upperLiquidationThresholdD9
            ) {
                rootVault.shutdown();
                isLiquidated = true;
            } else {
        ...
        address mainToken_ = mainToken;
        int256 singleOptionProfitETH = int256(D18) -
            int256(FullMath.mulDiv(startingOptionStrikeD9, D18, priceChangeD9));

        if (singleOptionProfitETH > 0) {
            uint256 optionProfit = FullMath.mulDiv(
                startingOptionMoney,
    ...
    function updateOracleParams(OracleParams calldata newOracleParams) external {
        _requireAdmin();
        require(
            (newOracleParams.slippageD9 <= D9) && (newOracleParams.oracleObservationDelta > 0),
    ...
    function _swapToToken(SwapParams memory params) internal returns (uint256 amountIn, uint256 amountOut) {
        ...
        bytes memory routerResult = params.spender.externalCall(
            address(swapRouter_),
