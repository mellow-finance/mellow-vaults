...
contract QuickPulseStrategyV2Helper {
    uint256 public constant Q96 = 2**96;

    function getStrategyParams(QuickPulseStrategyV2 strategy)
        public
        view
        returns (
            QuickPulseStrategyV2.ImmutableParams memory immutableParams,
            QuickPulseStrategyV2.MutableParams memory mutableParams
        )
    {
        {
            (IERC20Vault erc20Vault, IQuickSwapVault quickSwapVault, address router) = strategy.immutableParams();
            immutableParams = QuickPulseStrategyV2.ImmutableParams({
        ...
        }
        {
            ...
            mutableParams = QuickPulseStrategyV2.MutableParams({
    ...
    function calculateAmountForSwap(QuickPulseStrategyV2 strategy)
        public
        view
        returns (
            uint256 amountIn,
        ...
        )
    {
        ...
        {
            (QuickPulseStrategyV2.Interval memory interval, bool neededNewInterval) = strategy.calculateNewPosition(
                mutableParams,
                spotTick,
                pool,
                immutableParams.quickSwapVault.positionNft()
            );

            (delta, ) = immutableParams.quickSwapVault.tvl();
            if (!neededNewInterval) {
                (delta, ) = immutableParams.quickSwapVault.tvl();
                IQuickSwapHelper helper = immutableParams.quickSwapVault.helper();
                (, , , , , , uint128 liquidity, , , , ) = immutableParams.quickSwapVault.positionManager().positions(
                    immutableParams.quickSwapVault.positionNft()
                );
    ...
    function getSwapData(QuickPulseStrategyV2 strategy)
        public
        view
        returns (
            uint256 amountIn,
        ...
        )
    {
        ...
        address to;
        IERC20Vault erc20Vault;
        (amountIn, amountOutMinimum, from, to, erc20Vault) = calculateAmountForSwap(strategy);
